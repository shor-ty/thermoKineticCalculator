...
) const
{
    ...
    const scalar sigma = LJCD(species);

    //- Method of Hirschfelder et. al. 1954 [kg/m/s]
    if (method == "Hirschfelder")
    {
        return viscosityHirschfelder(species, T, MW, LennardJP, sigma);
    }
...
) const
{
    ...
    const scalar geometricConfig = 1; //= geometricConfig(species);
...
) const
{
    ...
    if (method == "Warnatz")
    {
        return (thermalConductivityWarnatz(species, T));
    }
    ...
    else if (method == "Polynomial")
    {
        ...
        const scalarField& polyCoeffs = thermalConductivityPolyCoeffs(species);
...
) const
{
    ...
    const scalar sigma = LJCD(species) * scalar(1e-10);

    //- Dimensionless temperature T*
    const scalar Ts = T * pow(LennardJP, -1);
...
) const
{
    ...
    {
        return binaryDiffusivityChapmanAndEnskog(species1, species2, T);
    }
    else if (method == "Polynomial")
    {
        //- Polynomial coefficients
        const scalarField& polyCoeffs =
            binaryDiffusivityPolyCoeffs(species1, species2);
...
) const
{
    ...
    const scalar& LJP2 = LJP(species2);
    ...
    forAll(species, s1)
    {
        ...
        forAll(species, s2)
        {
            ...
            for (size_t i = 0; i < n; ++i)
            {
                ...
                Dij.push_back
                (
                    log(binaryDiffusivity(s1, s2, Tv))
                );
            }
            ...
            //- Save the values
            binaryDiffusivityPolyCoeffs(s1, s2, x);
        }
    }
}
